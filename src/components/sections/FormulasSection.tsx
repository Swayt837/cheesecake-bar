    ...
    <section id="formulas" className="relative py-24 md:py-32 overflow-hidden" ref={ref}>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
          {formulas.map(({ key, icon, featured }, i) => (
            <motion.div
              key={key}
              initial={{ opacity: 0, y: 40 }}
              animate={isInView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.6, delay: i * 0.15 }}
              className={`group relative flex flex-col p-8 rounded-sm border transition-all duration-500
                ${featured
                  ? 'bg-gradient-to-b from-gold/10 to-leather-light border-gold/40 shadow-[0_0_30px_rgba(201,164,92,0.1)]'
                  : 'bg-leather-light/50 border-gold/10 hover:border-gold/30'
                }
            ...
            >
              ...
              {/* Ideal for */}
              <p className="text-sand/60 text-xs font-body mb-8">
